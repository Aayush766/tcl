...
import { 
  ...
  FiSearch, FiMessageSquare, FiArrowUpRight, FiLayers 
} from "react-icons/fi";
import { MouseEvent } from "react";
import Navbar from "../../../components/layout/navbar"; // Matches your project path
...
const services: ServiceItem[] = [
  { 
    ...
    color: "#ff0099", 
    size: "md:col-span-2",
    details: ["Modern React & Next.js frameworks", "Clean, premium, and responsive designs", "Smooth animations and user experiences"]
  },
  { 
    ...
    color: "#8a2be2", 
    size: "md:col-span-1",
    details: ["Custom ChatGPT & LLM setups", "Smart data search systems", "AI-powered internal features"]
  },
  { 
    title: "AI Automation", 
    ...
    color: "#00dbde", 
    size: "md:col-span-1",
    details: ["Custom workflow automation", "Smart data entry and processing", "Automating business logic steps"]
  },
  { 
    ...
    color: "#ff0099", 
    size: "md:col-span-1",
    details: ["User account & dashboard systems", "Secure subscription billing (Stripe)", "Flexible management portals"]
  },
  { 
    ...
    color: "#8a2be2", 
    size: "md:col-span-2",
    details: ["Structured schema setup", "Fast loading speeds for Google", "Performance tracking analytics"]
  },
  { 
    title: "AI Chatbots", 
    ...
    color: "#00dbde", 
    size: "md:col-span-1",
    details: ["24/7 intelligent automated replies", "Instant lead capture tools", "Multi-platform integration options"]
  },
];
    ...
    <main className="min-h-screen w-full bg-[#030303] text-white selection:bg-[#00DFD8]/20 selection:text-[#00DFD8] pb-32 pt-28 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 pt-12">
        ...
        <div className="mb-32 flex flex-col lg:flex-row lg:items-end justify-between gap-12">
          <div className="space-y-6 max-w-4xl">
            ...
            <p className="text-zinc-400 text-lg md:text-xl font-light leading-relaxed max-w-2xl pt-2">
              Modern digital solutions built to help your business grow. We build custom software, implement smart AI tools, and design websites that convert.
            </p>
          </div>
          ...
          <div className="lg:text-right flex-shrink-0">
            <p className="max-w-[280px] text-xs font-mono uppercase tracking-widest text-zinc-500 leading-relaxed lg:ml-auto border-l-2 lg:border-l-0 lg:border-r-2 border-zinc-800 pl-4 lg:pl-0 lg:pr-4">
              High-quality design and development tailored to your goals. // [EST 2026]
            </p>
          </div>
        </div>
        ...
        <motion.div 
          ...
          className="mt-20 rounded-3xl border border-white/5 bg-gradient-to-b from-zinc-950/40 to-transparent p-8 md:p-12 backdrop-blur-xl relative overflow-hidden group"
        >
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs font-mono text-[#00dbde]">
                <FiLayers className="animate-pulse" /> READY TO START
              </div>
              <h3 className="text-2xl md:text-3xl font-bold tracking-tight">Have a specific project in mind?</h3>
              <p className="text-zinc-400 text-sm max-w-xl font-light">
                Let's schedule a conversation to talk about your requirements and figure out how we can build it for you.
              </p>
            </div>
            <button className="relative group/btn overflow-hidden h-14 px-8 rounded-2xl bg-white text-black font-bold tracking-tight text-sm transition-transform active:scale-95 flex items-center gap-3">
              Get In Touch
              <FiArrowUpRight className="text-lg transition-transform group-hover/btn:translate-x-1 group-hover/btn:-translate-y-1" />
            </button>
          </div>
        </motion.div>
...
function ServiceCard({ service, index }: { service: ServiceItem; index: number }) {
  ...
  return (
    <motion.div
      ...
      className={`${service.size} group relative overflow-hidden rounded-3xl border border-white/5 bg-zinc-950/20 p-8 backdrop-blur-sm transition-all duration-500 hover:border-white/10 hover:bg-zinc-950/40`}
    >
      ...
      <div className="relative z-10 flex h-full flex-col justify-between min-h-[320px]">
        ...
        <div className="flex items-start justify-between">
          <div 
            className="flex h-14 w-14 items-center justify-center rounded-2xl bg-zinc-900/60 text-xl text-white transition-all duration-500 group-hover:scale-110 group-hover:bg-zinc-900"
            style={{ border: `1px solid ${service.color}25` }}
          >
            {service.icon}
          </div>
          <span className="font-mono text-[10px] tracking-widest text-zinc-600 group-hover:text-zinc-400 transition-colors">
            0{index + 1}
          </span>
        </div>
        ...
        {/* Card Metadata & Specs */}
        <div className="mt-12">
          <h2 className="text-2xl font-bold tracking-tight text-white mb-3 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-white group-hover:to-zinc-400 transition-all duration-300">
            {service.title}
          </h2>
          <p className="max-w-[260px] text-sm leading-relaxed text-zinc-400 transition-colors group-hover:text-zinc-300 mb-6 font-light">
            {service.desc}
          </p>
          ...
          <ul className="space-y-2 mb-8 opacity-0 max-h-0 pointer-events-none group-hover:opacity-100 group-hover:max-h-[100px] group-hover:pointer-events-auto transition-all duration-700 ease-in-out">
            {service.details.map((detail, dIdx) => (
              <li key={dIdx} className="text-[11px] font-mono text-zinc-500 flex items-center gap-2">
                <span className="h-1 w-1 rounded-full" style={{ backgroundColor: service.color }} />
                {detail}
              </li>
            ))}
          </ul>
          
          {/* Action Trigger Thread Line */}
          <div className="mt-auto pt-4 flex items-center gap-2 overflow-hidden">
            <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.2em] text-zinc-500 group-hover:text-white transition-colors duration-300">
              Learn More <FiArrowUpRight className="transition-transform group-hover:translate-x-0.5 group-hover:-translate-y-0.5" />
            </div>
            <div 
